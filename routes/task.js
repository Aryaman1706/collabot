...
router.post("/addTask/:id", [auth, lead], async (req, res) => {
  ...
  let project = await Project.findById(req.params.id).select("tasks team");
  ...
  // find the user
  const taskLead = await User.findOne({ email: req.body.taskLead }).select(
    "username email _id"
  );
  if (!taskLead) {
...
router.put("/addAssign/:id", [auth, taskLead], async (req, res) => {
  ...
  let task = await Task.findById(req.params.id).select("assignedTo");
...
router.put("/addReport/:id", [auth, taskLead], async (req, res) => {
  // find the task from req.params.id
  let task = await Task.findById(req.params.id).select("reportTo");
...
router.put("/taskLead/:id", auth, async (req, res) => {
  // find new lead
  const newLead = await User.findById(req.body._id).select(
    "email username _id"
  );
